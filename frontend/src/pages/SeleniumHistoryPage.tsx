...
function groupByVersion(tests: SeleniumTestRun[]): { version: string; tests: SeleniumTestRun[] }[] {
  const map = new Map<string, SeleniumTestRun[]>()
...
export default function SeleniumHistoryPage() {
  const navigate = useNavigate()
  const [page, setPage] = useState<SeleniumPage<SeleniumTestRun> | null>(null)
  ...
  useEffect(() => {
    fetchSeleniumAllLabels().then(setAllLabels).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetchSeleniumTests({
      page: pageNum, size: 15, sortBy, sortDir,
      browser: filterBrowser || undefined,
      label: filterLabel || undefined,
    })
      .then(setPage)
      .finally(() => setLoading(false))
  }, [pageNum, sortBy, sortDir, filterBrowser, filterLabel, viewMode])
  ...
  useEffect(() => {
    ...
    fetchSeleniumTests({
      page: 0, size: 500, sortBy, sortDir,
      browser: filterBrowser || undefined,
      label: filterLabel || undefined,
    })
      .then(p => {
  ...
  }, [viewMode, sortBy, sortDir, filterBrowser, filterLabel])
  ...
  async function handleVersionSave(id: number) {
    ...
    await updateSeleniumTestVersion(id, editingVersion.value)
  ...
  async function handleDelete(id: number) {
    if (!confirm(`Delete Selenium test #${id}?`)) return
    await deleteSeleniumTest(id)
    setPage((prev) => prev ? {
      ...prev,
  ...
  async function handleCancel(id: number) {
    ...
    const updateContent = (runs: SeleniumTestRun[]) =>
      runs.map((r) => r.id === id ? { ...r, status: 'CANCELLED' as const } : r)
    setPage((prev) => prev ? { ...prev, content: updateContent(prev.content) } : null)
    setFolderData(prev => updateContent(prev))
  }
      ...
      <tr key={run.id}>
            ...
            {editingVersion?.id === run.id ? (
              <div className="flex-row">
                <input type="text" value={editingVersion.value}
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleVersionSave(run.id)}
                  style={{ width: '80px' }} />
        ...
        <td>
          {run.status !== 'QUEUED' && run.status !== 'RUNNING' ? (
            <span>
              <span style={{ color: '#4ade80' }}>{run.loops > 1 ? run.passedIterations : run.passedInstances}</span>
              {' / '}
              <span style={{ color: '#f87171' }}>{run.loops > 1 ? run.failedIterations : run.failedInstances}</span>
            </span>
          ) : '-'}
        </td>
        <td>{run.meanStepDuration != null ? `${run.meanStepDuration.toFixed(0)} ms` : '-'}</td>
        <td>
          <div className="flex-row">
            {run.status === 'RUNNING' && (
              <button className="btn btn-secondary" style={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem' }}
                onClick={() => navigate(`/selenium/test/${run.id}`)}>Live</button>
            )}
            {run.status === 'QUEUED' && (
              <button className="btn btn-secondary" style={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem', color: '#f39c12' }}
                onClick={() => handleCancel(run.id)}>Cancel</button>
            )}
            {run.status !== 'RUNNING' && run.status !== 'QUEUED' && (
              <>
                <button className="btn btn-secondary" style={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem' }}
                  onClick={() => navigate(`/selenium/test/${run.id}`)}>View</button>
